    ...
    return (
      <div>
        <div className="d-flex justify-content-between">
          <div>
            {!this.state.deleted && (
              <p className="bold margin1x">{this.props.comment.user_name}</p>
            )}
          </div>
          ...
          {!this.state.deleted && user_id === this.props.comment.user_id && (
            <div className="d-flex">
              <i
                ...
                className="margin1x material-icons tiny gray70"
              >
                edit
              </i>
              <i
                onClick={this.handleDelete}
                className="material-icons tiny gray70"
              >
                delete_forever
              </i>
            </div>
          ...
          {this.state.update ? (
            ...
            >
              <InputGroup className="paddingy1" id="editbox">
                <Input
                  value={this.state.updateComment.body}
            ...
            <p className="padding1">{this.state.updateComment.body} </p>
          ) : null}
        </div>
      </div>
    );
